...
x_train , y_train = load_pickle(opt.P1_train_path, 1)
...
csv_logger = callbacks.CSVLogger(opt.P1_log_path)

filepath = opt.P1_model_path
...
len_train = get_data_length(opt.P1_train_path, batch_size=None)
len_test = get_data_length(opt.P1_test_path, batch_size=None)

train_dir = opt.P1_train_path
test_dir = opt.P1_test_path
...
if args.Loss == 'L_beta' or args.Loss ==  'sum_log_L_beta_adapt' or args.Loss == 'log_L_beta_adapt':
    ...
    beta_train = loadmat('/path/to/beta_train_fcn_L1.mat')
    beta_train = beta_train['Beta']

    beta_test = loadmat('/path/to/beta_test_fcn_L1.mat')
    beta_test = beta_test['Beta']
    
    train_generator = generator_beta(x_set = train_list, beta = beta_train[:,((args.level-1)*3)])
    test_generator = generator_beta(x_set = test_list, beta = beta_test[:,((args.level-1)*3)])
else:
